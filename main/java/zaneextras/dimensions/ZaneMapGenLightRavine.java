...
public class ZaneMapGenLightRavine extends MapGenBase {
	...
	protected void func_151540_a(long seed, int x, int z, Block[] block,
		...
		for (; p_151540_15_ < p_151540_16_; ++p_151540_15_) {
			double d12 = 1.5D + MathHelper
					.sin(p_151540_15_ * (float) Math.PI / p_151540_16_)
					* p_151540_12_ * 1.0F;
			...
			if (flag1 || rand.nextInt(4) != 0) {
				...
				if (p_151540_6_ >= d4 - 16.0D - d12 * 2.0D
						...
						&& p_151540_10_ <= d5 + 16.0D + d12 * 2.0D) {
					int i4 = MathHelper.floor_double(p_151540_6_ - d12) - x * 16
							- 1;
					int l1 = MathHelper.floor_double(p_151540_6_ + d12) - x * 16
							+ 1;
					int j4 = MathHelper.floor_double(p_151540_8_ - d6) - 1;
					int i2 = MathHelper.floor_double(p_151540_8_ + d6) + 1;
					int k4 = MathHelper.floor_double(p_151540_10_ - d12)
							- z * 16 - 1;
					int j2 = MathHelper.floor_double(p_151540_10_ + d12)
							- z * 16 + 1;
					...
					for (k2 = i4; !flag2 && k2 < l1; ++k2) {
						for (int l2 = k4; !flag2 && l2 < j2; ++l2) {
							for (int i3 = i2 + 1; !flag2
									&& i3 >= j4 - 1; --i3) {
								j3 = (k2 * 16 + l2) * 256 + i3;
								
								if (i3 >= 0 && i3 < 256) {
									if (this.isOceanBlock(block, j3, k2, i3, l2,
											x, z)) {
										flag2 = true;
									}
							...
							for (j3 = k4; j3 < j2; ++j3) {
								...
								if (d13 * d13 + d14 * d14 < 1.0D) {
									for (int l3 = i2 - 1; l3 >= j4; --l3) {
										...
										if ((d13 * d13 + d14 * d14)
												* this.field_75046_d[l3]
												+ d11 * d11 / 6.0D < 1.0D) {
											if (this.isTopBlock(block, k3, k2,
													l3, j3, x, z)) {
												flag = true;
											}
											this.digBlock(block, k3, k2, l3, j3,
													x, z, flag);
										}
										--k3;
									}
	...
	private boolean isTopBlock(Block[] data, int index, int x, int y, int z,
			int chunkX, int chunkZ) {
		BiomeGenBase biome = this.worldObj.getBiomeGenForCoords(x + chunkX * 16,
				z + chunkZ * 16);
	...
	protected void digBlock(Block[] data, int index, int x, int y, int z,
		...
		if (block == BlockList.lightStone || block == filler || block == top) {
			if (y < 10) {
				data[index] = BlockList.lightStone;
			} else {
				data[index] = null;
