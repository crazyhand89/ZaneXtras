...
import zaneextras.noise.ZaneGradient;
import zaneextras.noise.ZaneNoiseModule;

public class ChunkProviderLight extends ChunkProviderGenerate {
	
	final Block topBlockID = BlockList.lightGrassDirt;
	final byte topBlockMeta = 0;
	final Block fillBlockID = BlockList.lightStone;
	final byte fillBlockMeta = 0;
	final Block lowerBlockID = BlockList.lightStone;
	final byte lowerBlockMeta = 0;
	
	private final Random rand;
	
	private final ZaneNoiseModule noiseGen1;
	private final ZaneNoiseModule noiseGen2;
	private final ZaneNoiseModule noiseGen3;
	private final ZaneNoiseModule noiseGen4;
	
	private final World worldObj;
	
	private BiomeGenBase[] biomesForGeneration = { ZaneBiomeList.lightBiome };
	
	private MapGenBase caveGenerator = new ZaneMapGenCavesLight();
	...
	private ZaneMapGenLightRavine ravineGenerator = new ZaneMapGenLightRavine();
	
	private ZaneWorldGenLightDungeons dungeonGenerator = new ZaneWorldGenLightDungeons();
	...
	public ChunkProviderLight(World par1World, long par2, boolean par4) {
		...
		this.noiseGen1 = new ZaneGradient(this.rand.nextLong(), 4, 0.25F);
		this.noiseGen2 = new ZaneGradient(this.rand.nextLong(), 4, 0.25F);
		this.noiseGen3 = new ZaneGradient(this.rand.nextLong(), 1, 0.25F);
		this.noiseGen4 = new ZaneGradient(this.rand.nextLong(), 1, 0.25F);
	}
	...
	public void generateTerrain(int chunkX, int chunkZ, Block[] idArray,
		...
		for (int x = 0; x < ChunkProviderLight.CHUNK_SIZE_X; x++) {
			for (int z = 0; z < ChunkProviderLight.CHUNK_SIZE_Z; z++) {
				final double d = this.noiseGen1.getNoise(x + chunkX * 16,
						z + chunkZ * 16) * 8;
				final double d2 = this.noiseGen2.getNoise(x + chunkX * 16,
						z + chunkZ * 16) * 24;
				double d3 = this.noiseGen3.getNoise(x + chunkX * 16,
						z + chunkZ * 16) - 0.1;
				d3 *= 4;
				...
				for (int y = 0; y < ChunkProviderLight.CHUNK_SIZE_Y; y++) {
					if (y < ChunkProviderLight.MID_HEIGHT + yDev) {
		...
		for (int var8 = 0; var8 < 16; ++var8) {
			for (int var9 = 0; var9 < 16; ++var9) {
				final int var12 = (int) (this.noiseGen4
						.getNoise(var8 + par1 * 16, var9 * par2 * 16) / 3.0D
						+ 3.0D + this.rand.nextDouble() * 0.25D);
				int var13 = -1;
	...
	public void populate(IChunkProvider par1IChunkProvider, int par2,
		...
		if (this.rand.nextInt(4) == 0 && TerrainGen.populate(par1IChunkProvider,
		...
		for (k1 = 0; doGen && k1 < 8; ++k1) {
			...
			new ZaneWorldGenLightDungeons().generate(this.worldObj, this.rand,
					l1, i2, j2);
		}
