...
public class LightLeaves extends BlockLeavesBase implements IShearable {
	...
	public LightLeaves() {
		...
		this.setBlockName(ModInfo.MODID + "_lightleaves");
		this.setBlockTextureName(ModInfo.MODID + ":leaves_light");
	...
	public void breakBlock(World world, int x, int y, int z, Block par5,
		...
		if (world.checkChunksExist(x - bounds, y - bounds, z - bounds,
		...
		if ((meta & 8) != 0 && (meta & 4) == 0) {
			...
			if (world.checkChunksExist(x - i1, y - i1, z - i1, x + i1, y + i1,
				...
				for (l1 = -b0; l1 <= b0; ++l1) {
					for (i2 = -b0; i2 <= b0; ++i2) {
						for (j2 = -b0; j2 <= b0; ++j2) {
							Block block = world.getBlock(x + l1, y + i2,
									z + j2);
							
							if (block != null && block.canSustainLeaves(world,
									x + l1, y + i2, z + j2)) {
								this.adjacentTreeBlocks[(l1 + k1) * j1
										+ (i2 + k1) * b1 + j2 + k1] = 0;
							} else if (block != null && block.isLeaves(world,
									x + l1, y + i2, z + j2)) {
								this.adjacentTreeBlocks[(l1 + k1) * j1
										+ (i2 + k1) * b1 + j2 + k1] = -2;
							} else {
	...
	@Override
	public boolean isShearable(ItemStack item, IBlockAccess world, int x, int y,
			int z) {
		return true;
	}
